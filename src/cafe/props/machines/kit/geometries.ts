...
/**
 * Procedural primitive kit for the brewing-equipment group.
 *
 * Follows the same pattern as the room shell: a handful of shared unit
 * geometries are re-used (scaled) for the very common box/cylinder/sphere
 * cases, while anything with a baked profile (tapers, torus handles, discs)
 * allocates its own small geometry. `disposeObjectTree` disposes every
 * non-shared geometry plus all materials, so tearing a rig down leaks nothing.
 */

const SHARED_FLAG = 'sharedBrewingKitGeometry';
...
/** Sphere of `radius`, squashed along y by `yScale` (domes, knobs). */
...
/** Cone pointing up (+y), used for lids, spouts and hopper caps. */
...
/**
 * Torus ring in the local XY plane (hole facing ±z). Handles, coils, rims.
 * Arcs start at the +x axis and sweep counter-clockwise.
 */
export function torus(
  ...
  const mesh = new THREE.Mesh(
    new THREE.TorusGeometry(radius, tube, 12, Math.max(10, Math.round(arc * 24)), arc),
    material,
  );
  mesh.position.set(x, y, z);
...
/** Flat disc; default orientation faces up (+y), or faces −z with axis 'z'. */
export function disc(
  ...
  const mesh = new THREE.Mesh(new THREE.CircleGeometry(radius, 28), material);
  if (axis === 'y') mesh.rotation.x = -Math.PI / 2;
  if (axis === 'z') mesh.rotation.y = Math.PI;
  mesh.position.set(x, y, z);
  return mesh;
}
...
/**
 * Marks translucent glass so it never casts a solid shadow blob. Registration
 * forces castShadow=true on everything; the rig clears the flag on first era
 * application (and once via microtask right after build).
 */
...
/** Emissive UI surfaces (screens, LEDs, flames): never cast shadows. */
